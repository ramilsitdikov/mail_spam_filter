...
# нужно прочитать оттуда данные!
csvFile = codecs.open("datadata.csv",  "rb", "utf-8")
ads = []
grades = []
words = []
csvFile.readline()
...
def words_of_string(text):
    ...
    # функция разрезает предложение на слова и возвращает список слов (без их
    # повторений)
    all_words = []
    k = text.split('.')
...
def popular_words(list_sentence,  N):
    ...
    # проходит по всем словам, добавляем их в словарь, считаем сколько их в
    # списке и берем N штук
    dic = {}
